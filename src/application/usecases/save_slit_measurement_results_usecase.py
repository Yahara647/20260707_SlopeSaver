# src/application/usecases/save_slit_measurement_results_usecase.py
...
class SaveSlitMeasurementResultsUseCase:
    """
    スリット光計測の結果アグリゲートをまとめて保存するユースケース。

    - execute() 呼び出し時に base_dir を指定する
    - base_dir/YYYYMMDD/ を作成
    - その中に YYYYMMDD_HHMMSS_mmmm/ を作成
    - app_config.json / calib.json / result.json を保存
    """
